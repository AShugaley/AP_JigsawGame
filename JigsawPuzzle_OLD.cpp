...
JigsawPuzzleOLD::JigsawPuzzleOLD(vector<PuzzlePiece> pieces){
...
JigsawPuzzleOLD::JigsawPuzzleOLD(string& inputFilePath, string& outputFilePath) : outputFile(outputFilePath), cannotComputeSolution(false),lastRowIndex(-1), lastColIndex(-1) ,piecesMap(PuzzlePiecesMap()){
    ifstream inputFile;
    ...
    int numElements = JigsawPuzzleOLD::readFirstLine(inputFile);
    if (numElements == -1){
    ...
    while (std::getline(inputFile, line)){
        JigsawPuzzleOLD::readPuzzlePieceLine(line);
    }

    inputFile.close();

    // Reaching here means the first line was valid and we succeeded to read the input file

    for (PuzzlePiece& p : this->correctInputPieces){
        this->validatePuzzlePiece(p);
        // if the puzzle piece is not valid we will move here to the wrong input pieces vector
    }
...
int JigsawPuzzleOLD::readFirstLine(ifstream &openInputFileStream) {
    string line;
    std::getline(openInputFileStream, line);
    vector<string> lineWords = JigsawPuzzleOLD::split(line, '=');
    if (lineWords.size() != 2){
        return -1;
    }
...
vector<string> JigsawPuzzleOLD::split(const string &s, char delimiter) {
    vector<string> words;
...
void JigsawPuzzleOLD::readPuzzlePieceLine(string& line){
    vector<string> pieceInfo = JigsawPuzzleOLD::split(line, ' ');
...
void JigsawPuzzleOLD::validatePuzzlePiece(PuzzlePiece& piece){
    int id = piece.getISD();
...
void JigsawPuzzleOLD::updateMissingIDs(){
...
void JigsawPuzzleOLD::updateWrongElementsIDs(){
    vector<PuzzlePiece>& pieces = this->correctInputPieces;
    for (PuzzlePiece& p : pieces){
...
void JigsawPuzzleOLD::writeErrorsToOutput(ofstream& openOutputFileStream){
...
bool JigsawPuzzleOLD::printSolutionToFile(bool solved){
    ofstream outputFile;
    outputFile.open(this->outputFile);
    if (!outputFile.is_open()){
        cout << OUTPUT_FILE_NOT_OPEN;
...
vector<int> JigsawPuzzleOLD::hasAllCorners(){
...
bool JigsawPuzzleOLD::hasEnoughEdges(){
    int count = 0;
    for(int i = 0; i<this->numOfElements; i++){
...
bool JigsawPuzzleOLD::isSumEdgesZero(){
    int sum = 0;
    for(int i = 0; i<this->numOfElements; i++){
        sum += correctInputPieces[i].edgesSum();
    }
    
    return sum == 0;
}

bool JigsawPuzzleOLD::isSumHorizontalEdgesZero(){
    int sum = 0;
    for(int i = 0; i < this->numOfElements; i++){
...
bool JigsawPuzzleOLD::isSumVerticalEdgesZero(){
    int sum = 0;
    for(int i = 0; i < this->numOfElements; i++){
...
bool JigsawPuzzleOLD::initSolveGame(){
    return printSolutionToFile(initSolve());
}

bool JigsawPuzzleOLD::isLegalPuzzle(){
    ofstream outputFile;
    outputFile.open(this->outputFile);
    bool legal = true;
    if (!outputFile.is_open()){
...
pair<int,int> JigsawPuzzleOLD::getNextPos(int i, int j){
    if(j!=lastColIndex){
...
vector<pair<int,int> > JigsawPuzzleOLD::getPossibleDimensions(int numOfPieces) {
...
PuzzleRequirement JigsawPuzzleOLD::getReq(int i, int j){
    int l, r, t,b;
...
void JigsawPuzzleOLD::initSolMatrix(){
...
bool JigsawPuzzleOLD::initSolve(){
    this->piecesMap = PuzzlePiecesMap(this->correctInputPieces);
...
bool JigsawPuzzleOLD::solveRec(pair<int,int> nextPos){
        int i = nextPos.first;
