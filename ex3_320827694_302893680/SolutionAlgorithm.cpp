...
bool SolutionAlgorithm::solveGame(int numOfThreads, bool rotate){
    ...
    while(!possibleDimensions.empty() || !threads.empty()){
        ...
        while((currentNumOfThreads < numOfThreads) && !possibleDimensions.empty()){
            ...
            threads.back().f = std::async(std::launch::async, &SolutionAlgorithmRunningSuite::solveGamePair,SolutionAlgorithmRunningSuite(factory.getJigsawGame(pieces), factory.getPuzzleMap(pieces)) , p.first, p.second, &finished);
            currentNumOfThreads++;
        }
