...
bool SolutionAlgorithmRunningSuite::solveRec(pair<int,int> nextPos, bool * finished){
    if(*finished)
        return true;
    int i = nextPos.first;
    ...
    while(p!=nullptr){
        ...
        bool solved = solveRec(game->getNextPos(i,j), finished);
        if (solved){
            return true;
        }
        else{
...
bool SolutionAlgorithmRunningSuite::solveGame(vector<pair<int,int> > possibleDimensions){
    ...
    for (auto& p : possibleDimensions){
        ...
        if(solveRec(topLeftCorner, &f) == true){
            return true;
        }
    }
    return false;
...
pair<bool,unique_ptr<JigsawGameInterface> > SolutionAlgorithmRunningSuite::solveGamePair(int i, int j, bool * finished){
    pair<int,int> topLeftCorner = pair<int,int>(0,0);
    this->game->initiateSolutionTry(pair<int,int>(i,j));
    if(solveRec(topLeftCorner, finished) == true){
